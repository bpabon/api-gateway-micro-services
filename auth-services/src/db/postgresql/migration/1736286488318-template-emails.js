export class TemplateEmails1736286488318 {

    async up(queryRunner) {
        const largeHTMLString = `<!DOCTYPE html>
        ...
        <a style="color: #333333;font-weight: bold;">Importante:</a> El enlace para cambiar su contraseña estará disponible solo durante la próxima hora.</p>
                                        </td>
                                    </tr>
                                    <tr>
        ...
        await queryRunner.query(`
            ...
            CREATE TABLE "template_emails" (
                "id" SERIAL PRIMARY KEY, -- ID de la plantilla
                "status" estado_enum DEFAULT 'Activo', -- Inactivo, Activo
                "codigo" VARCHAR(100) NOT NULL, -- Código único de cada una de las plantillas (único)
                "version" VARCHAR(10) NOT NULL, -- Versión de la plantilla ejemplo V1   
                "template" TEXT, -- Texto de la plantilla en HTML5
            ...
            COMMENT ON TABLE "template_emails" IS 'Tabla para los diferentes templates de los correos electrónicos que se envíen desde el software';
            COMMENT ON COLUMN "template_emails"."id" IS 'ID de la plantilla';
            COMMENT ON COLUMN "template_emails"."status" IS 'Inactivo, Activo';
            COMMENT ON COLUMN "template_emails"."codigo" IS ' Código único de cada una de las plantillas (único con la versión)';
            COMMENT ON COLUMN "template_emails"."version" IS 'Versión de la plantilla ejemplo V1  ';
            COMMENT ON COLUMN "template_emails"."template" IS 'Texto de la plantilla en HTML5';
            COMMENT ON COLUMN "template_emails"."created_at" IS 'Fecha de creación del registro';
            CREATE UNIQUE INDEX unique_template_version ON template_emails (codigo, version);
            ` );
            await queryRunner.query(` INSERT INTO template_emails (status,codigo,version,template) 
                VALUES ('Activo','RECOVERY_PASSWORD','v1', $1); `, [largeHTMLString]);
    }

    async down(queryRunner) {
        await queryRunner.query(`DROP TABLE "template_emails"`);
        await queryRunner.query(`DROP TYPE estado_enum`);
